...
n = 50;  % Phase intervals
n2 = 1; % Torque intervals
n3 = 2; % Velocity intervals
tdot_max = 2500;
...
torque_vec = zeros(length(n));
i_phase_vec = zeros(length(n));
...
parfor x = 1:n
        [t, i_d, i_q] = motor_fun_voltage(sqrt(2)*160, phase(x), 1000);
        mag = norm([i_d, i_q]);
        torque_vec(x) = t*(mag<mag_max);
        mag_vec(x) = norm([i_d, i_q]);
end

figure;plot(i_phase_vec, torque_vec);
